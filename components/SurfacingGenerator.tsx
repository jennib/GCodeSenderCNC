...
const SurfacingGenerator: React.FC<SurfacingGeneratorProps> = ({ onParamsChange, toolLibrary, unit, settings }) => {
    ...
    const handleParamChange = (field: keyof typeof params, value: string) => {
        ...
        setParams(newParams);
        onParamsChange(field, numValue);
    };

    return (
        <div className='space-y-4'>
            <ToolSelector selectedId={params.toolId} onChange={(id) => { setParams(prev => ({ ...prev, toolId: id })); onParamsChange('toolId', id); }} unit={unit} toolLibrary={toolLibrary} />
            ...
            <RadioGroup label='Milling Direction' options={[{ value: 'horizontal', label: 'Horizontal (X)' }, { value: 'vertical', label: 'Vertical (Y)' }]} selected={params.direction} onChange={val => { setParams(prev => ({ ...prev, direction: val })); onParamsChange('direction', val); }} />
