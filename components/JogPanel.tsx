...
import { ArrowUp, ArrowDown, ArrowLeft, ArrowRight, Pin, RotateCw, RotateCcw, PowerOff, Probe } from './Icons.tsx';
...
interface JogPanelProps {
    ...
    onProbe: (axes: string, offsets: { x?: number, y?: number, z?: number }) => void;
    jogStep: number;
...
const JogPanel: React.FC<JogPanelProps> = ({
    isConnected,
    machineState,
    onJog,
    onHome,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className='bg-background p-3 rounded-md'>
                    ...
                    <div className='grid grid-cols-3 grid-rows-3 gap-2'>
                        <div className='col-start-1 row-start-1' /> {/* empty */}
                        ...
                        <div className='col-start-1 row-start-3' /> {/* empty */}
                    ...
                    <div className='mt-3 border-t border-secondary pt-3'>
                        ...
                        <div className='grid grid-cols-1 gap-2 text-sm'>
                            <button onClick={() => onHome('all')} disabled={isControlDisabled} className='p-2 bg-secondary rounded hover:bg-secondary-focus disabled:opacity-50 font-bold'>Home All ($H)</button>
                        </div>
                    </div>
                </div>
                        ...
                        <h4 className='text-sm font-bold text-text-secondary mb-2'>Set Zero</h4>
                         <div className='space-y-2 text-sm'>
                            <div className='grid grid-cols-3 gap-2'>
                                <button onClick={() => onSetZero('all')} disabled={isControlDisabled} className='p-2 bg-secondary rounded hover:bg-secondary-focus disabled:opacity-50'>Zero All</button>
                                <button onClick={() => onSetZero('xy')} disabled={isControlDisabled} className='p-2 bg-secondary rounded hover:bg-secondary-focus disabled:opacity-50'>Zero XY</button>
                                <button onClick={() => onSetZero('z')} disabled={isControlDisabled} className='p-2 bg-secondary rounded hover:bg-secondary-focus disabled:opacity-50'>Zero Z</button>
                            </div>
                        </div>
                    </div>
                    <div className='bg-background p-3 rounded-md'>
                        ...
                        <div className='flex bg-secondary rounded-md p-1'>
                            <button onClick={() => onUnitChange('mm')} disabled={isControlDisabled} className={`w-1/2 p-1 rounded-md text-sm font-semibold transition-colors ${unit === 'mm' ? 'bg-primary text-white' : 'hover:bg-secondary-focus'} disabled:opacity-50 disabled:cursor-not-allowed`}>mm</button>
                            <button onClick={() => onUnitChange('in')} disabled={isControlDisabled} className={`w-1/2 p-1 rounded-md text-sm font-semibold transition-colors ${unit === 'in' ? 'bg-primary text-white' : 'hover:bg-secondary-focus'} disabled:opacity-50 disabled:cursor-not-allowed`}>in</button>
                        </div>
                    </div>
                    <div className='bg-background p-3 rounded-md'>
                        ...
                        <div className='grid grid-cols-2 gap-4'>
                             <div className='space-y-2'>
                                <div className='flex items-center gap-2'>
                                    <label htmlFor='probe-offset-x' className='w-4 text-sm font-bold text-red-400'>X</label>
                                    <input id='probe-offset-x' type='number' step={unit === 'mm' ? '0.01' : '0.001'} value={probeOffsetX} onChange={e => setProbeOffsetX(parseFloat(e.target.value))} disabled={isControlDisabled} className='w-full bg-secondary border-secondary rounded-md py-1 px-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary disabled:opacity-50' aria-label='Probe X Offset' />
                                </div>
                                 <div className='flex items-center gap-2'>
                                    <label htmlFor='probe-offset-y' className='w-4 text-sm font-bold text-green-400'>Y</label>
                                    <input id='probe-offset-y' type='number' step={unit === 'mm' ? '0.01' : '0.001'} value={probeOffsetY} onChange={e => setProbeOffsetY(parseFloat(e.target.value))} disabled={isControlDisabled} className='w-full bg-secondary border-secondary rounded-md py-1 px-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary disabled:opacity-50' aria-label='Probe Y Offset' />
                                </div>
                                 <div className='flex items-center gap-2'>
                                    <label htmlFor='probe-offset-z' className='w-4 text-sm font-bold text-blue-400'>Z</label>
                                    <input id='probe-offset-z' type='number' step={unit === 'mm' ? '0.01' : '0.001'} value={probeOffsetZ} onChange={e => setProbeOffsetZ(parseFloat(e.target.value))} disabled={isControlDisabled} className='w-full bg-secondary border-secondary rounded-md py-1 px-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary disabled:opacity-50' aria-label='Probe Z Offset (Plate Thickness)' />
                                </div>
                            </div>
                            <div className='grid grid-cols-2 gap-2 text-sm'>
                                <button onClick={() => onProbe('X', { x: probeOffsetX })} disabled={isControlDisabled} className='p-2 bg-primary text-white font-semibold rounded hover:bg-primary-focus disabled:opacity-50'>Probe X</button>
                                <button onClick={() => onProbe('Y', { y: probeOffsetY })} disabled={isControlDisabled} className='p-2 bg-primary text-white font-semibold rounded hover:bg-primary-focus disabled:opacity-50'>Probe Y</button>
                                <button onClick={() => onProbe('Z', { z: probeOffsetZ })} disabled={isControlDisabled} className='p-2 bg-primary text-white font-semibold rounded hover:bg-primary-focus disabled:opacity-50 flex items-center justify-center gap-1'><Probe className='w-4 h-4' /> Probe Z</button>
                                <button onClick={() => onProbe('XY', { x: probeOffsetX, y: probeOffsetY })} disabled={isControlDisabled} className='p-2 bg-primary text-white font-semibold rounded hover:bg-primary-focus disabled:opacity-50'>Probe XY</button>
                            </div>
                        </div>
                    </div>
                    <div className='bg-background p-3 rounded-md'>
